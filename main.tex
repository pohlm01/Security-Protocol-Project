...
\begin{enumerate}[label={SR\arabic*:}, leftmargin=3\parindent]
    ...
    % \item \label{sr:reloadAuthCard}
    % The reload terminal authenticates the card.
    ...
    % \item \label{sr:cardReloadMessageConf}
    % Communication between a card and a reload terminal is confidential.
    % \item \label{sr:connectReload}
    % Terminal needs to be connected to back-end periodically.
    ...
    % \item \label{sr:cardPOSMessageConf}
    % Communication between a card and a POS terminal is confidential.
    ...
    Money transfer between a card and POS terminal must behave atomic. % Card tear attacks
    \item \label{sr:disablePOS}
    A POS terminal can be deactivated remotely by trustful parties.
    % \item \label{sr:connectPOS}
    % A POS terminal needs to be connected to the back-end periodically.
